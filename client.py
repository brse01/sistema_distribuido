...
while True:
    ...
    try:
        ...
        while True:
            ...
            resposta, op = data.split('@')                        
            log('Mensagem "%s" recebida de "%s"' % (data, str(client_address)))            
            if data:
                if int(op) == 1:
                    #data = ("Confirmando o recebimento de \'%s\'" % data)
                    log('Respondendo com a mensagem "%s"' % (horario_static))
                    connection.sendall(horario_static)
                else:
                    horario_static = int(horario_static) + int(resposta)                    
                    log('Minha nova hora é "%d"' % horario_static)    
            else:
